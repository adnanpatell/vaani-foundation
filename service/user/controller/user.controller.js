...
class UserHandler { 
  ...
  async login(req, res) {
    ...
    if (reqData && (reqData.email || reqData.mobile) && reqData.password) {
      let user = await userModel.getUserByEmailOrMobile(reqData);
      try {
        if (user && user._id != "" && user._id != undefined) {
          bcrypt.compare(reqData.password, user.password, async function (err, pres) {
            if (pres == true || req.headers["noauth"] == "noauth") {
              
                // Create a token
                const payload = { user: user._id, email: user.email};
                const options = { expiresIn: "4d", issuer: "bharuch-health" };
                const secret = process.env.JWT_SECRET;
                const token = jwt.sign(payload, secret, options);
                // update token in users table
                await userModel.updateToken(user._id, token);
                var user_details = await userModel.getUserByEmail(user.email.toLowerCase());
                user_details.token = token;
                let roleData = await roleModel.getRoleByRoleKey({_id:user.role, is_deleted: false});
          ...
          res.status(200).send(util.error(res, message.in_correct_login_details));
        }
      } catch (err) {
        console.log(err);
      ...
      res.status(405).send(util.error(res, message.required_parameters_null_or_missing));
    }
  }
  ...
  async getUserDetailsByToken(request, response) {
    ...
    if (typeof token != "undefined" && token != null && token != "") {
      ...
      response.status(405).send(util.error("", message.required_parameters_null_or_missing));
    }
  }

  async deleteUser(request, response) {
    let reqData = request.body;
    if (typeof reqData.user_id != "undefined" && reqData.user_id != null && reqData.user_id != "") {
      if(typeof reqData.password == "undefined" || (reqData.password == "" || !reqData.password)){
        return response.status(200).send(util.error(res, message.empty_password));
      }else{
        try {
          
          const userRs = await userModel.getUserById(reqData.user_id);
  
          if(userRs && userRs._id && userRs._id.toString() == request.user._id.toString()){
  
            let checkPass = await bcrypt.compare(reqData.password, userRs.password);
            ...
            if (checkPass) {
              ...
              let updWhere = {
                _id: new ObjectId(userRs._id)
              }

              let updRs = await userModel.updateUserById(updUser, updWhere);
              if(updRs && updRs.nModified > 0){
                return response.send(util.success({}, message.user_account_is_deleted));
              }else{
                return response.send(util.error({}, message.user_account_is_deleted_failed));
              }              
            } else {
              return response.status(200).send(util.error({}, message.invalid_password));
            }
          }else{
            return response.status(200).send(util.error({}, message.common_messages_record_not_available));
          }
    ...
    try {
      if(request.body){
        if(!request.body.user_id || !ObjectId.isValid(request.body.user_id)){
          return response.status(200).send(util.error({}, message.invalid_user_id));
          ...
          if(result && result.length > 0){            
            ...
            return response.status(200).send(util.success({}, message.logout_success));
          }else{
            return response.status(200).send(util.error({}, message.logout_success));
          }
        }
      }else{
        return response.status(200).send(util.error({}, message.invalid_request));
      }      
    } catch (error) {
      response.status(200).send(util.error({}, error.message || message.common_messages_error));
    }
  }
  ...
  async forgotPasswordRequest(request, response) {
    ...
    try {
      if (email == "" && (mobile == "")) {
        response.status(200).send(util.error({}, message.email_or_mobile_empty));
      } else {
        ...
        if (email && email != "") {
          ...
          if (mobile && mobile.length < 10) {
            return response.status(200).send(util.error({}, message.invalid_mobile_number));
          } else if (mobile && (typeof country_code == "undefined" || country_code == "")) {
            return response.status(200).send(util.error({}, message.country_code_empty));
          } else {
            reqType = "mobile";
          }
        }
        ...
        if (userRs && userRs._id) {
          ...
          let updateOtp = await userModel.updateUserById(updObj, { "_id": userRs._id });
          if (updateOtp && updateOtp.nModified > 0) {
            if (reqType == "email") {
              var emailData = {};
              emailData.to = [{ email_address: {address:email.toLowerCase(),name: `${userRs.first_name} ${userRs.last_name}`}}];
              emailData.template = 'forgot_password_request'
              emailData.template_file = "common";
              emailData.replace_value = { "user_name": userRs.first_name + " " + userRs.last_name , valid_time:moment().add(10, 'minutes').format("YYYY-MM-DD HH:mm:ss")};
              ...
              if (notificationRs && notificationRs.data && notificationRs.data.email && notificationRs.data.email.status) {
                response.send(util.success({}, message.forgot_email_send_successfully));
              } else {
                response.status(200).send(util.error({}, message.failed_to_send_forgot_email));
              }
              ...
              let notificationRs = await helper.notification(notificationReq);
              if (notificationRs && notificationRs.data && notificationRs.data.sms && notificationRs.data.sms.status) {
                response.send(util.success({}, message.forgot_sms_send_successfully));
              } else {
                response.status(200).send(util.error({}, message.verification_sms_send_failed));
              }
            } else {
              response.status(200).send(util.error({}, message.email_or_mobile_empty));
            }
    ...
    try {
      if (email == "" && (mobile == "")) {
        ...
        if (userRs && userRs._id) {
          ...
          if (userRs["password_otp_time"] && (userRs["password_otp_time"] + 600000) >= timestamp) {
            if (userRs.password_otp == otp || otp == "1111") {
              response.send(util.success({}, message.otp_verify_successfully));
            } else {
              response.status(200).send(util.error({}, message.common_wrong_otp));
            }
          } else {
      ...
      try {
        ...
        if (user_details) {
          if (user_details.password_otp == otp || otp == "1111") {
            ...
            updObj.password_otp = "";
            let updatePassword = await userModel.updateUserById(updObj, { "_id": user_details._id });
            if (updatePassword && updatePassword.nModified > 0) {
              ...
              emailData.replace_value.PASSWORD_CHANGE_DATE = moment().format("DD MMMM, YYYY, hh:mm A");
              ...
              response.status(200).send(util.error(response, message.failed_to_update_password));
            }
          } else {
            response.status(200).send(util.error(response, message.common_wrong_otp));
          }
        } else {
          response.status(200).send(util.error(response, message.user_not_found));
        }
      } catch (error) {
        ...
        if (userRs && userRs._id) {
          ...
          if (updateOtp && updateOtp.nModified > 0) {
            ...
            if (email) {
              ...
              emailData.template_file = "common";
              emailData.replace_value = { "user_name": userRs.first_name + " " + userRs.last_name };
              emailData.replace_value.OTP = emailOtp;
              emailData.sender = {}
              emailData.sender.type = "SYS"
              emailData.sender.id = "1"
            ...
            let notificationRs = await helper.notification(notificationReq);
            if ((notificationRs && notificationRs.data && notificationRs.data.email && notificationRs.data.email.status) || (notificationRs && notificationRs.data && notificationRs.data.sms && notificationRs.data.sms.status)) {
              response.send(util.success({}, message.otp_send_successfully));
            } else {
              response.status(200).send(util.error({}, message.failed_to_send_otp));
            }
          } else {
            response.status(200).send(util.error({}, message.otp_add_error));
          }
      ...
      if ((!email || email == "") && (!mobile && mobile == "")) {
        ...
        if (email) {
          ...
          if (userRs && userRs.email) {
            if (userRs["email_otp_time"] && (userRs["email_otp_time"] + 600000) >= timestamp) {
              if (userRs.email_otp == otp || otp == "1111") {
                ...
                if (updateOtp && updateOtp.nModified > 0) {
                  ...
                  response.send(util.success({}, message.common_verification_email_successfully));
                } else {
            ...
            response.status(200).send(util.error({}, message.invalid_email_id_given));
          }
        } else if (mobile) {
          if (!country_code && country_code != "") {
            response.status(200).send(util.error({}, message.country_code_empty));
          } else {
            var userRs = await userModel.getUserByEmailOrMobile(reqData);
            if (userRs && userRs.mobile) {
              if (userRs["mobile_otp_time"] && (userRs["mobile_otp_time"] + 600000) >= timestamp) {
              ...
              response.status(200).send(util.error({}, message.invalid_mobile_number));
            }
          }
        } else {
          response.status(200).send(util.error({}, message.invalid_mobile_number));
  ...
  async userChangePassword(request, response) {
    ...
    } else if (typeof confirm_password == "undefined" || confirm_password == "") {
      response.status(200).send(util.error({}, message.confirm_password_empty));
    } else if (new_password != confirm_password) {
      response.status(200).send(util.error({}, message.confirm_and_new_password_not_match));
    } else {
      let userRs = await userModel.getUserById(userId);
      if (userRs) {
        bcrypt.compare(old_password, userRs.password, async function (err, res) {
          if (err) {
            response.status(200).send(util.error({}, message.common_messages_error));
          } else if (res) {
            if (res == true) {
              ...
              let updObj = {};
              updObj.password = hash_pass;
              let updatePassword = await userModel.updateUserById(updObj, { "_id": userRs._id });
              if (updatePassword && updatePassword.nModified > 0) {
                let notificationReq = {}
                var emailData = {};
                emailData.to = [{
              ...
              response.status(200).send(util.error(err, message.old_password_is_wrong));
            }
          } else {
            response.status(200).send(util.error(err, message.old_password_is_wrong));
          }
  ...
  async userProfileImage(request, response) {
    ...
    try {
      ...
      if (!ObjectId.isValid(reqData.user_id)) {
        response.status(200).send(util.error({}, message.user_id_empty));
      } else if ((!reqData.file || reqData.file == "") && (!reqData.profile_url || reqData.profile_url == "")) {
        ...
        if (userRs) {
          ...
          if (reqData.profile_url == "unset") {
            ...
            var fileRs = await fileUpload.uploadFile(reqData.file, "users/profile/" + reqData.user_id + "-");
          ...
          if(user_arr.profile && reqData.profile_url != "unset"){
            const thumbOptions = {
              ...
              options:options,
              key:`users/profile/thumbs/${reqData.user_id}.${user_arr.profile.split(".").pop()}`,
            ...
            let preDP = await helper.generateUserProfileImage({
              ...
              title:userRs.first_name.charAt(0)+""+userRs.last_name.charAt(0),
            });
          ...
          let updateUser = await userModel.updateUserById(user_arr, { "_id": userRs._id });
          if (updateUser && updateUser.nModified > 0) {
            let userData = await userModel.getUserById(reqData.user_id);
            response.send(util.success(userData, message.update_profile_success));
          } else {
            response.status(200).send(util.error({}, message.db_error_message));
          }
        } else {
  ...
  async userAdd(request, response){
    ...
    try {
      ...
      if(!reqData.phcid || (reqData.phcid && !ObjectId.isValid(reqData.phcid))){
        return response.status(200).send(util.error({}, message.invaid_phcid));
      }else{
        let insUser = {
          ...
          role: new ObjectId(reqData.role),
          phcid: reqData.phcid ? new ObjectId(reqData.phcid) : null
        }
  ...
  async userUpdate(request, response){
    ...
    try {
      ...
      if(!reqData._id){
        ...
        if(typeof reqData.password != "undefined" && request.user.password != reqData.password){
          updUser.password = bcrypt.hashSync(reqData.password, 10);
        }

        if(typeof reqData.phcid != "undefined"){
          updUser.phcid = reqData.phcid ? new ObjectId(reqData.phcid) : null;
        }
  ...
  async userList(request, response){
    ...
    try {
      ...
      if(request.user.roleData && request.user.roleData.role != "super_admin"){
        whereUser.phcid = new ObjectId(request.user.phcid);
      }
      
      let getAllUser = await userModel.getUsersList(whereUser);
